...
namespace App\Library\mailchimp;
...
if (!class_exists('Mailchimp')) {
    ...
    class Mailchimp
    {
         ...
         * Field = "<strong>aim</strong>"		 * Valid Op(erations): <strong>open</strong> / <strong>noopen</strong> / <strong>click</strong> / <strong>noclick</strong>
         ...
         */
        function lists($filters = array(), $start = 0, $limit = 10000)
        {
         ...
         * array choices optional kind of - an array of strings to use as the choices for radio and dropdown type field
        ...
        function callServer($method, $params)
        {
            ...
            $url = 'https://' . $host . '/' . $this->version . '/' . $method;

            if ($method == 'lists') {
                return $this->call_list($url, array());
            }
        }

        function call_list($url, $post = array())
        {
            $ch = curl_init();
            ...
            if (!empty($result)) {
                ...
                if (isset($responses->lists) && !empty($responses->lists)) {
                    $response = $this->set_array_list($responses->lists);
                }
            } else {
                $response = array();
            }
        ...
        function set_array_list($lists)
        {
            $main_arr['data'] = array();
            if (!empty($lists)) {
                foreach ($lists as $list) {
                    $main_arr['data'][] = array('id' => $list->id, 'name' => $list->name);
                }
            }
            return $main_arr;
        }
        ...
        function MailChimpAddContact($email, $status, $listId, $apiKey, $mergeFields)
        {
            $data = array(
                'apikey' => $apiKey,
                'email_address' => $email,
            ...
            $mchApi = curl_init(); // initialize cURL connection
            curl_setopt($mchApi, CURLOPT_URL, 'https://' . substr($apiKey, strpos($apiKey, '-') + 1) . '.api.mailchimp.com/3.0/lists/' . $listId . '/members/' . md5(strtolower($data['email_address'])));
            ...
            curl_setopt($mchApi, CURLOPT_RETURNTRANSFER, true); // return the API response
            curl_setopt($mchApi, CURLOPT_CUSTOMREQUEST, 'PUT'); // method PUT
            curl_setopt($mchApi, CURLOPT_TIMEOUT, 10);
            curl_setopt($mchApi, CURLOPT_POST, true);
            curl_setopt($mchApi, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($mchApi, CURLOPT_POSTFIELDS, json_encode($data)); // send data in json
